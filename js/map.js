...
function initMap() {
  const mapEl = document.getElementById('bondia-map');
  ...
  const customIcon = L.divIcon({
    ...
    html: `<div style="
      ...
      box-shadow:0 4px 18px rgba(194,84,26,0.55);
  ...
  // ── Segundo marcador: Panadería y Pastelería Denisse ──────────────
  const denisseIcon = L.divIcon({
    className: '',
    html: `<div style="
      width:44px;height:44px;
      background:#7B4FB5;
      ...
      transform:rotate(-45deg);
      display:flex;align-items:center;justify-content:center;
      box-shadow:0 4px 18px rgba(123,79,181,0.55);
      border:3px solid white;
    "><span style="
      transform:rotate(45deg);
      font-family:'Playfair Display',serif;
      ...
      color:white;line-height:1.2;text-align:center;
    ">Den<br>isse</span></div>`,
    iconSize:    [44, 44],
    iconAnchor:  [22, 44],
    popupAnchor: [0, -52],
  });

  const marker2 = L.marker([LAT2, LNG2], { icon: denisseIcon }).addTo(map);
  ...
  const bounds = L.latLngBounds([[LAT, LNG], [LAT2, LNG2]]);
